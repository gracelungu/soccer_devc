...
Fountain.prototype.init = function(defs, x, y) {
  this.f = defs;
  this.location = createVector(x || width * this.f.x, y || height * this.f.y);
  if (this.f.dxy)
    this.dxy = createVector(width * this.f.dxy[0], height * this.f.dxy[1]);
  ...
  if (this.f.acceleration)
    this.f.acceleration = createVector(
      this.f.acceleration[0],
      this.f.acceleration[1]
    );
  if (this.f.file) {
